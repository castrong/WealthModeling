...
plot_graph(g) = TikzGraphs.plot(g, Layouts.Spring(), node_style="draw, rounded corners, fill=blue!10")
...
md"""
## Simulate a simple 1-state system on the graph
Create functions for how edge values are assigned and how the vertex state depends on those edge values.
"""

# ╔═╡ db948f4a-a4a7-4616-9beb-ea78e3e72454
begin
	function edge_state!(e, v_s, v_d, p, t)
	    # usually e, v_s, v_d are arrays, hence we use the broadcasting operator.
	    e .= v_s - v_d
	    nothing
	end
	
	function vertex_dynamics!(dv, v, edges, p, t)
	...
	# of the state
	nd_edge = StaticEdge(f! = edge_state!, dim = 1)
	nd_vertex = ODEVertex(f! = vertex_dynamics!, dim = 1)
end

# ╔═╡ 074cfb65-5537-40ee-8b29-55c1b913f98b
md"""
Setup the ODE problem and solve, then plot
"""

# ╔═╡ a860a067-8550-4bd4-8aa8-44838978f77b
begin
	# It all gets wrapped up into an object we can pass into the ODE solver
	nd = network_dynamics(nd_vertex, nd_edge, g)
	x0 = randn(num_nodes) # random initial conditions
	time_interval = (0., 4.)
	ode_prob = ODEProblem(nd, x0, time_interval)
...
begin
	# It all gets wrapped up into an object we can pass into the ODE solver
	nd_wealth = network_dynamics(nd_vertex_wealth_income, nd_edge_income, g)
	x0_wealth = interleave_vectors(rand(nv(g)), zeros(nv(g))) #randn(num_nodes) # random initial conditions
	time_interval_wealth = (0., 10.)
	ode_prob_wealth = ODEProblem(nd_wealth, x0_wealth, time_interval_wealth)
	sol_wealth = solve(ode_prob_wealth, Tsit5());	
end
...
md"""
## Create a graph with two separate groups linked at a single point
"""

# ╔═╡ 0980107c-2898-458d-9b9c-8f75c4e9fa02
md"""
n_connect
"""

# ╔═╡ b3a9764e-7a5e-42e7-a727-6744ae8c3816
@bind n_connect Slider(1:10)

# ╔═╡ a6721601-fccf-42cd-aa7b-6fb02d6d0cf7
md"""
n1
"""

# ╔═╡ 1935f303-fc34-4605-a713-6a6349526574
@bind n1 Slider(1:10)

# ╔═╡ c6d4eb37-db94-47a6-9770-7b449cd748d3
md"""
n2
"""

# ╔═╡ d0487b24-3b35-4cfd-85a7-642ee40488a4
@bind n2 Slider(1:10)

# ╔═╡ 68b35386-0f9e-432f-9805-4745aeaa737f
begin
	# Generate two graphs
	g1 = wheel_graph(n1) # wheel_graph vs complete_graph to have less connections 
	g2 = wheel_graph(n2) 

	# Combine the graphs, then connect them with n_connect connections
...
md"""
## Simulate a simple income + wealth system on the graph with two groups
"""

# ╔═╡ 863ce51a-7c98-4487-9e0a-25931a14878f
begin
		# Group the graph and dynamics into one object
		nd_segregation = network_dynamics(nd_vertex_wealth_income, nd_edge_income, segregated_g)
		
		# Set up the initial conditions
		income_disparity = 0.5 
		start_income = [rand(n1); rand(n2)*income_disparity]
		start_wealth = zeros(n1 + n2)
		x0_segregation = interleave_vectors(start_income, start_wealth)
end

# ╔═╡ 70c5b45d-9a91-4e51-bc60-b21db71f1559
begin
	# Simulate the system
	time_interval_segregated = (0., 50.)
	ode_prob_segregation = ODEProblem(nd_segregation, x0_segregation, time_interval_segregated)
	sol_segregation = solve(ode_prob_segregation, Tsit5());	
end
...
md"""
Compute then plot the wealth gap
"""

# ╔═╡ 7e913617-3967-4fe0-b225-328de916c57e
begin
	avg_wealth1 = mean(sol_segregation[2:2:n1*2, :], dims=1)
	avg_wealth2 = mean(sol_segregation[n1*2+2:2:(n1+n2)*2, :], dims=1)
	gap = avg_wealth1 ./ avg_wealth2
	Plots.plot(sol_segregation.t[2:end], gap[2:end])
end
...
[[NetworkDynamics]]
deps = ["DiffEqBase", "LightGraphs", "LinearAlgebra", "NLsolve", "SparseArrays"]
git-tree-sha1 = "7ed42ea6e2eabf23f581bd387fdf65c2e7db7575"
