...
# ╔═╡ 397c90fe-5b85-11ec-16ea-9fef8fe70670
using NetworkDynamics, DifferentialEquations, Plots, Graphs, OrdinaryDiffEq, PlutoUI, Statistics, GraphPlot

# ╔═╡ 8f5339b9-0238-4927-b4d6-024ea8589fd4
md"""
## Graph Generation
"""

# ╔═╡ bba1b870-0c1b-4662-8fc0-4a164627dfcd
md"""
##### Helper functions for calculating probabilities. 
Equations for each probability are given without derivation here. See the writeup or GraphGeneration.jl for derivations.
"""

# ╔═╡ 3a80d803-ede5-4459-beea-79fd5f839fd7
md"""
$P(C = 1) = \frac{n_c}{n}$
"""

# ╔═╡ 1ce7b35e-45e3-4ce5-a999-defc8e27ecc9
P_C_1(n_c, n) = nothing

# ╔═╡ f4b72b32-06f6-43b3-97d5-418eee32da91
md"""
$P(C = 0) = 1 - P(C = 1)$
"""

# ╔═╡ c6942452-f422-4f07-b46f-123f5c6fb067
P_C_0(n, n_c) = nothing

# ╔═╡ 07c3fdfd-a1e9-4fbb-96bb-b6ba0c4ef600
md"""
$P(K = 1) = \frac{\binom{n_1}{2} + \binom{n_2}{2}}{\binom{n}{2}}$
"""

# ╔═╡ 192d7ac2-22ed-4e09-980a-acd2a2e64539
P_K_1(n₀, n₁) = nothing

# ╔═╡ beedb98b-7050-462b-ac02-2ac7a4ea175c
md"""
$P(K = 0) = \frac{n_1 n_2}{n} = 1 - P(K = 1)$
"""

# ╔═╡ c209963f-2e61-4a32-9b4e-a2b2307c09c6
P_K_0(n₀, n₁) = nothing

# ╔═╡ 2f4eac78-855b-4ccb-9799-af8b689bbff9
md"""
$P(K = 1 \mid C = 1) = R_h P(K = 1)$
"""

# ╔═╡ f086a387-82b2-4e32-b03c-b7bf5b9af08a
P_K_1_C_1(n₀, n₁, Rₕ) = nothing

# ╔═╡ b13e018c-b573-4e88-b453-14459e5fd247
md"""
$P(K = 0 \mid C = 1) = 1 - P(K = 0 \mid C = 1)$
"""

# ╔═╡ 21614d7e-f403-45e3-8325-904262f3a408
P_K_0_C_1(n₀, n₁, Rₕ) = nothing

# ╔═╡ cd04f76d-885d-4583-870d-66c39aa0d85d
md"""
$P(C = 1 \mid K = 1) = R_h P(C = 1)$
"""

# ╔═╡ 770daa72-fe2b-4e60-bd9f-04cb8a6e3d7a
P_C_1_K_1(n, n_c, Rₕ) = nothing

# ╔═╡ 3c28af2e-7fc8-4fa4-ab63-d8d82dfcaa12
md"""
$P(C = 0 \mid K = 1) = 1 - P(C = 1 \mid K = 1)$
"""

# ╔═╡ d63de89b-a56f-4daf-9c6b-25ba25e9dfa0
P_C_0_K_1(n, n_c, R_h) = nothing

# ╔═╡ c36c905f-908f-424e-8d16-db31ddb57db0
md"""
$P(C = 1 \mid K = 0) = \frac{P(C = 1) - P(C = 1 \mid K = 1)P(K = 1)}{P(K = 0)}$
"""

# ╔═╡ 293f8211-8205-4a2e-87dd-c605c2360787
P_C_1_K_0(n₀, n₁, n_c, Rₕ) = nothing

# ╔═╡ 4ab16cd1-1db2-46ca-afa4-b87ba0b8b0ee
md"""
$P(C = 0 \mid K = 0) = 1 - P(C = 1 \mid K = 0)$
"""

# ╔═╡ 59f9ebc9-4295-4e5b-b01b-b08ce561eb76
P_C_0_K_0(n₀, n₁, n_c, Rₕ) = nothing

# ╔═╡ cef53e62-edb5-429c-9aa2-e3c4c2afb8e2
md"""
$\max \left(0, \frac{P(C=1) - P(K=0)}{P(C=1)P(K=1)}\right) \le R_h \le \min \left( \frac{1}{P(C=1)}, \frac{1}{P(K = 1)} \right)$
"""

# ╔═╡ e874f1c5-65eb-419a-9111-4131fe683a77
function Rₕ_bounds(n₀, n₁, n_c)
	n = n₀ + n₁
	lower_bound = max(0, nothing)
	upper_bound = min(nothing, nothing)
end

# ╔═╡ 8ae6e098-fe6e-4301-9343-54df0cb1e77d
md"""
##### An initial graph generation algorithm.
"""

# ╔═╡ 49f0df49-b15e-4986-9115-a959da5bd185
struct KindGraph
	graph::Graph
	kinds::Vector{Int}
end

# ╔═╡ 4173dc7d-a338-4ff7-a22b-905880fe78c6
# K(i, j) = 1 if nodes i and j are the same kind, 0 otherwise
K(i, j, kinds) = nothing

# ╔═╡ 5b825fab-35d9-42a6-bb84-4de7cb039b93
function generate_graph(n₀, n₁, n_c, Rₕ)
	# Build an adjacency matrix for the graph
	n = n₀ + n₁
	A = zeros(n, n)
	
	# The kind will be 0 for the first n₀ and 1 for the remaining n₁ nodes. 
	kinds = nothing # TODO

	# The probabilities we need are P(C = 1 | K = 0) and P(C = 1 | K = 1)
	p_c_1_k_0 = P_C_1_K_0(n₀, n₁, n_c, Rₕ)
	p_c_1_k_1 = P_C_1_K_1(n_c, n₀ + n₁)
	for i = 1:n
		for j = i+1:n
			# The probability of nodes i and j being connected will
			# depend on whether they are the same kind
			same_kind = K(i, j, kinds) == 1
			prob_connected = same_kind ? p_c_1_k_1 : p_c_1_k_0
			connected = rand() < prob_connected 
			A_adj[i, j] = connected ? 1 : 0
		end
	end
	return KindGraph(Graph(A_adj + A_adj'), kinds)
end

# ╔═╡ 42addfdc-abdc-4e4e-a025-bffb3e1d8904
md"""
##### Test and visualize generating graphs with different properties
"""

# ╔═╡ f6308f1b-247f-47d1-beaa-a9568e88a5c8
md"""
 $n_0$ $(@bind n0 Slider(0:5:100, show_value=true, default=5))
"""

# ╔═╡ 52f90000-b3e5-4094-9438-4ee792d65854
md"""
 $n_1$ $(@bind n1 Slider(1:100, show_value=true, default=5))
"""

# ╔═╡ 5f4f42fb-3516-493d-b8da-71cc50b3b558
md"""
 $n_c$ Min: 0 Max: n-1 = $(n0 + n1 - 1) $(@bind nc Slider(0:n0+n1 - 1, show_value=true, default=3)) 
"""

# ╔═╡ 102af6b1-4afc-4274-80ed-10d888a2cc8c
# A helper function to plot graphs
function plot_graph(kind_graph::KindGraph)
	g, kinds = kind_graph.graph, kind_graph.kinds
	nodesize = [Graphs.outdegree(g, v)*2 + 10 for v in Graphs.vertices(g)]
	color_0, color_1 = colorant"yellow", colorant"pink" #distinguishable_colors(2, colorant"blue")
	node_colors = [(kind == 0 ? color_0 : color_1) for kind in kinds]
	gplot(g, nodefillc=node_colors)
end

# ╔═╡ 51795113-f506-45e8-a5b6-d5b9efedf97e
Rₕₘᵢₙ, Rₕₘₐₓ = Rₕ_bounds(n0, n1, nc)

# ╔═╡ 4ba342fa-dc59-4856-a5a4-073dcbf3ced3
md"""
 $R_h$ min: $Rₕₘᵢₙ, max: $Rₕₘₐₓ  $(@bind Rₕ NumberField(Rₕₘᵢₙ:0.05:Rₕₘₐₓ, default=Rₕₘₐₓ)) 
"""

# ╔═╡ d32e4ee3-3a7d-46d7-b79c-1e45135f9aa5
md"""
# Simulating graph dynamics
"""

# ╔═╡ eab70b66-0390-4599-aaa8-37086ff8007e
function simulate(graph::KindGraph, nd_edge_fcn, nd_vertex_fcn, x₀, time_interval)
	# Wrap the graph an dynamics into a NetworkDynamics Object
	g, kinds = graph.graph, graph.kinds
	nd = network_dynamics(nd_vertex_fcn, nd_edge_fcn, g)

	# Create an ODE Problem, then solve
	ode_prob = ODEProblem(nd, x₀, time_interval)
	sol = solve(ode_prob, Tsit5())
	return sol
end

# ╔═╡ 86f54645-3bcf-4046-906e-833a4e39358a
md"""
##### Define the dynamics we will use
"""

# ╔═╡ 64a06d23-45d1-4108-b074-c0e3ca0b0e1a
begin
	wealth_appreciation = 0.0
	savings_rate = 0.5
	income_growth_rate = 0.00
	
	function edge_fcn_income!(e, v_s, v_d, p, t)
	    # usually e, v_s, v_d are arrays, hence we use the broadcasting operator.
		e .= v_s[1] - v_d[1]
	    nothing
	end
	
	function vertex_fcn_wealth_income!(dv, v, edges, p, t)
	    # usually v, edges are arrays, hence we use the broadcasting operator .
		dv .= 0.
	    for e in edges
	        dv[1] += 1/length(edges)*e[1]
	    end
		dv[1] += income_growth_rate * v[1]
		dv[2] = wealth_appreciation * v[2] + savings_rate * v[1] # dwealth/dt = 0.01 * wealth + 0.5 * income
	    nothing
	end

	# Wrap the functions into a NetworkDynamics object which also tracks the dimension 
	# of the state
	nd_edge_income = StaticEdge(f = edge_fcn_income!, dim = 1)
	nd_vertex_wealth_income = ODEVertex(f = vertex_fcn_wealth_income!, dim = 2, sym = [:income, :wealth])
end

# ╔═╡ 5b6f4130-2d2b-4fe5-be66-fa148feeb64f
md"""
##### Generating initial conditions
"""

# ╔═╡ e0653408-c6ca-4081-9ab3-e541a8a12a00
interleave_vectors(x, y) = [x y]'[:] # trick to interleave two vectors

# ╔═╡ 353c7c03-90d8-4bf4-8e42-4c943987468b
function generate_x₀(n₀, n₁, income_ratio)
	income_0 = [rand(n₀); rand(n₁)*income_ratio]
	wealth_0 = zeros(n₀ + n₁)
	return interleave_vectors(income_0, wealth_0)
end

# ╔═╡ 60c3de01-e041-40ff-a772-9b94e137c3dc
md"""
##### Graphing Helpers
"""

# ╔═╡ 6dbd07b9-5f2b-4ccf-bbf5-e2cb727e15a0
# Assumes the first n₁ are type 0, and the rest are type 1.
function plot_wealth_gap(n₁, n₂, sol)
	avg_wealth1 = mean(sol[2:2:n₁*2, :], dims=1)
	avg_wealth2 = mean(sol[n₁*2+2:2:(n₁+n₂)*2, :], dims=1)
	gap = avg_wealth1 ./ avg_wealth2
	Plots.plot(sol.t[2:end], gap[2:end])
end

# ╔═╡ e96d264f-d277-45e7-b5b7-7c38d9bf829f
# Function to plot "income" and "wealth" for each node
function plot_variable(sol, variable_str)
	Plots.plot(sol, vars = syms_containing(nd_segregation, variable_str), fmt = :png)
end

# ╔═╡ 3dd80cfb-4182-4142-98a5-2db51a015c0b
md"""
##### Test Simulation Code
"""

# ╔═╡ a82b1cb8-f70c-4aac-9448-c791da6a218e
begin
	g = KindGraph(wheel_graph(4), [0, 0, 1, 1])
	x₀ = generate_x₀(2, 2, 0.5)
	sol = simulate(g, nd_edge_income, nd_vertex_wealth_income, x₀, (0., 10.))
	plot_wealth_gap(2, 2, sol)
end


# ╔═╡ 91352ad6-ad09-4ce5-8c8e-80b5968bbb21
# Define the graph parameters
begin
	n₀ = 
	n₁ = 
	n_c = 
	homophily_bounds = Rₕ_bounds(n₀, n₁, n_c)
end

# ╔═╡ fdc77c26-e479-49b7-ac1b-c5ac5a029d0b


# ╔═╡ 8c4ce2af-ba76-49c3-88c7-d425178c2706


# ╔═╡ 25b46414-8d5b-48e5-972f-bcb4563c0134


# ╔═╡ bcfea50a-3214-4fd5-8fe0-99dc4aa0be0d
# Visualize the resulting wealth gap trend
plot_wealth_gap(sol, n₀, n₁)
...
[[NetworkDynamics]]
deps = ["DiffEqBase", "Graphs", "LinearAlgebra", "NLsolve", "SparseArrays"]
git-tree-sha1 = "0a8ec24a9b0594b3d4b882f783fb046da797397f"
