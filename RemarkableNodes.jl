...
# ╔═╡ c5c30dd4-2ca6-4522-8c68-ef506c44e733
md"""
### Creating a graph
"""

# ╔═╡ 7b21e95f-7752-4b3e-9568-2dbe9cd95241
...
# ╔═╡ 20778ddb-df18-440b-94cf-9272ab766689
# Old notes
# (1) Set number of edges / node --> P(C(i, j) = 1 ∣ i, j)
# (2) Set number of group A --> P(K(i, j) = 1 ∣ i, j), P(K(i, j) = 0 ∣ i, j)
# (3) Set P(C(i, j) = 1 ∣ K(i, j) = 1, i, j)
# (4) Set P(C(i, j) = 1 ∣ K(i, j) = 0, i, j)

# ╔═╡ 02b7f723-bcf6-4073-a8c3-41135f41ada8
md"""
### Create a graph with homophily.

First, select the number of people of kind 0 and the number of people of kind 1.
"""
...
md"""
Next select the average degree you would like each node to have.
"""
...
md"""
$P(C = 1) = n_c \cdot \frac{n}{2} \cdot \frac{1}{\binom{n}{2}} = \frac{n_c}{n-1}$
"""

# ╔═╡ b1f12268-4689-4c80-ae5f-8209906d71dd
# Calculate P(C = 1) to set the upper bound on the Homophily ratio
P_C_1 = n_c * n/2 * 1/(binomial(n, 2))
# experienced edges / nodes * nodes / (experienced edges / edge) * 1/(possible edges)

# ╔═╡ 40a43015-19fc-424e-82db-861db4be4ddc
md"""
$P(K = 0) = \frac{n_0 n_1}{n \choose 2}$
"""

# ╔═╡ 84e69a96-9e59-470a-94fc-94b2fb4178e1
P_K_0 = n₀ * n₁ / binomial(n, 2)

# ╔═╡ a76ff7fb-e992-4232-90cb-a37e1cd3b3a9
md"""
$P(K = 1) = \frac{\binom{n_0}{2} + \binom{n_1}{2}}{\binom{n}{2}} = 1 - P(K = 0)$
"""
...
md"""
Next, we calculate the parameters that derive from these and that we'll use to directly generate the graph.
"""

# ╔═╡ 0e94ab59-4e98-4174-bf42-ef9911018974
@assert P_K_0 + P_K_1 == 1 "Should add to 1"


# ╔═╡ 6d8237aa-8786-4fb3-873f-e310076a6cb6
md"""
...
P_C_0_K_0 = 1 - P_C_1_K_0

# ╔═╡ 89fdd570-ad50-4636-85c7-a0ba21f86d74
md"""
Set the kind of each node, then generate your adjacency matrix based on that kind.
"""

# ╔═╡ 5631b467-7ed7-4c33-a663-249684065120
begin
	kinds = [zeros(n₀); ones(n₁)]
	K(i, j, kinds) = (kinds[i] == kinds[j] ? 1 : 0)
	...
	for i = 1:n
		for j = (i+1):n
			...
			# Equivalent to 
			# if rand() < prob_connected
			# 	return 1
			# else 
			#	return 0
			# end
			A_adj[i, j] = (rand() < prob_connected) ? 1 : 0
		end
	end
...
begin
	# # Size the nodes by their degree
	# nodesize = [Graphs.outdegree(g, v) + 1 for v in Graphs.vertices(g)]
	# color_0, color_1 = distinguishable_colors(2, colorant"blue")
	# node_colors = [repeat([color_0], n₀); repeat([color_1], n₁)]
	# gplot(graph, nodelabel=1:n, nodesize=nodesize, nodefillc=node_colors)
end
...
begin
	# Visualization
	# Color the nodes 
	# larger n, generate a bunch of networks, graph of proportion of 
	x = [5]
	C = [1, 2, 3, 4];
	repeat(x, 5)
end
